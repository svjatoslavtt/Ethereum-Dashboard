...
function* getTokens(action: { payload: { address: string }, type: ActionTypesUnion }) {
	try {
		const { address } = action.payload;
		const { data } = yield call(request, process.env.REACT_APP_API + `/v1/1/address/${address}/balances_v2/`);
