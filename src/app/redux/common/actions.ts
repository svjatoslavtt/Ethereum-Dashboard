...
export const Actions = {
	...
	loginSuccess: (payload: { address: string }) => action(ActionTypes.LOGIN_SUCCESS, payload),
	loginFailed: (payload: any) => action(ActionTypes.LOGIN_FAILED, payload),

	logout: () => action(ActionTypes.LOGOUT),

	getBalancesRequest: (payload: { address: string }) => action(ActionTypes.GET_BALANCES_REQUEST, payload),
	getBalancesSuccess: (payload: any) => action(ActionTypes.GET_BALANCES_SUCCESS, payload),
	getBalancesFailed: (payload: any) => action(ActionTypes.GET_BALANCES_FAILED, payload),

	switchAccount: (payload: { address: string }) => action(ActionTypes.SWITCH_ACCOUNT, payload),
